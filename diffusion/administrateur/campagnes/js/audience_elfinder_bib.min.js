(function(a) {
    window.elFinder = function(b, c) {
            ...
            lang: "en",
            cssClass: "",
            commands: ["open","addtoplaylist","addtoplaylistmen","addtoplaylistwomen", "reload", "home", "up", "back", "forward", "getfile", "quicklook", "download", "rm", "duplicate", "rename", "mkdir", "mkfile", "upload", "copy", "cut", "paste", "edit", "extract", "archive", "search", "info", "view", "resize", "sort"],
            commandsOptions: {
                getfile: {
                    onlyURL: !0,
                    multiple: !1,
            ...
            contextmenu: {
                navbar: ["add"],
                cwd: ["add"],
                files: ["getfile", "|", "addtoplaylist","|","addtoplaylistwomen","|","addtoplaylistmen"]
            },
            ...
            .i18.en = {
                ...
                messages: {
                    ...
                    cmdopen: "Open",
                    cmdaddtoplaylist:"add to play list",
                    cmdaddtoplaylistmen:"add to play list (male version)",
                    cmdaddtoplaylistwomen:"add to play list (female version)",
                    cmdpaste: "Paste",
        ...
        function(b) {
            ...
            this.disableOnSearch = !0, this.updateOnSelect = !1, this.handlers = {
                ...
                elFinder.prototype.commands.addtoplaylistwomen = function() {
            this.exec = function() {

                //fonction de l'ajout à la playliste de diffusion

                var filename=$(".ui-selected").text();
                if(filename.indexOf(".")>=0)
                {
                var path= "..\\files\\files\\"+$(".elfinder-path").text()+'\\'+$(".ui-selected").text();
                if(path.indexOf(".mpeg")>0||path.indexOf(".mp4")>0||path.indexOf(".wav")>0)
                {
                var video='\
                        <video id="'+path+'" width="400" controls style=" display: none;">\
                          <source src="'+path+'" type="video/mp4">\
                          Your browser does not support HTML5 video.\
                        </video>';

                var idpath="duration"+filename;
                var ul= '<tr>\
                <td>  <button class="btn btn-xs" onclick="$(this).parent().parent().remove();"><span class="icon-remove"></span></button></td>\
                 <td><span class="filename">'+filename+video+'</span></td>\
                 <td><span id="'+idpath+'" class="duration" value="'+idpath+'" style="font-size:11px;"></span>\
                 <input type="hidden" name="file" id="'+path+'" value="'+path+'"/>\
                 </td></tr>';
              
                $("#addtbodywomen").append(ul);
                var vid=document.getElementById(path);
                vid.ondurationchange = function() {
                    var seconds=Math.floor(vid.duration);
                    var dur="00 : "+seconds;
                    if(seconds>=60) 
                    {
                        ...
                        if(minutes>=60) 
                        {
                         ...
                         document.getElementById(idpath).innerHTML+=dur;
                    };
                    }
                    else {
                        var ul= '<tr>\
                         <td>  <button class="btn btn-xs" onclick="$(this).parent().parent().remove();"><span class="icon-remove"></span></button></td>\
                         <td><span class="filename">'+filename+'</span></td><td><input type="hidden" name="file" id="'+path+'" value="'+path+'"/></td></tr>';
                           $("#addtbodywomen").append(ul);
                    }
              
                }
                else alert("Sélection invalide, seuls les fichiers sont permis.");
        ...
        elFinder.prototype.commands.addtoplaylistmen = function() {
            this.exec = function() {
        ...
        elFinder.prototype.commands.addtoplaylist = function() {
            this.exec = function() {
